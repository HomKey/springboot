...
/**
 * Created by LuHj on 2018/9/3.
 */
public class MySerialReader extends Observable implements Runnable,SerialPortEventListener {
    ...
    public static final String PORT = "COM3"; // 端口
...
//        String port=odao.selectNumberById(15);
        ...
        params.put( MySerialReader.PARAMS_PORT, MySerialReader.PORT ); // 端口名称
        params.put( MySerialReader.PARAMS_RATE, rate ); // 波特率
        params.put( MySerialReader.PARAMS_DATABITS,dataBit  ); // 数据位
        params.put( MySerialReader.PARAMS_STOPBITS, stopBit ); // 停止位
        params.put( MySerialReader.PARAMS_PARITY, parityInt ); // 无奇偶校验
        params.put( MySerialReader.PARAMS_TIMEOUT, 100 ); // 设备超时时间 1秒
        params.put( MySerialReader.PARAMS_DELAY, 100 ); // 端口数据准备时间 1秒
        try {
            open(params);//打开串口
...
//            addObserver(cf);
    ...
    public static void main(String[] args) {
        MySerialReader mySerialReader = new MySerialReader();
        mySerialReader.openSerialPort("111");
    }
